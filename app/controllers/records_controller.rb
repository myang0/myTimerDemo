class RecordsController < ApplicationController
    ...
    def index
        ...
        if request.fullpath == '/stats/year'
            @successes = Record.group_by_month(:created_at, reverse: true).where(uname: session[:user_name],
                                                                                 created_at: (t - 365.day)..t,
                                                                                 success: true).sum(:duration)

            @failures = Record.group_by_month(:created_at, reverse: true).where(uname: session[:user_name],
                                                                                created_at: (t - 365.day)..t,
                                                                                success: false).sum(:duration)
        # All records from today to a month ago
        elsif request.fullpath == '/stats/month'
            @successes = Record.group_by_day(:created_at, reverse: true).where(uname: session[:user_name],
                                                                               created_at: (t - 30.day)..t,
                                                                               success: true).sum(:duration)

            @failures = Record.group_by_day(:created_at, reverse: true).where(uname: session[:user_name],
                                                                              created_at: (t - 30.day)..t,
                                                                              success: false).sum(:duration)
        ...
        # All records from today
            ...
            @successes = Record.group_by_hour(:created_at, reverse: true).where(uname: session[:user_name],
                                                                                created_at: (t - 1.day)..t,
                                                                                success: true).sum(:duration)

            @failures = Record.group_by_hour(:created_at, reverse: true).where(uname: session[:user_name],
                                                                               created_at: (t - 1.day)..t,
                                                                               success: false).sum(:duration)
